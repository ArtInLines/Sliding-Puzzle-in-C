...
int main() {
    ...
    while (is_player_turn && !is_solved(len, board)) {
        ...
        while (play_turn_return_code != SUCCESS) {
            ...
            play_turn_return_code = play_turn(direction, column_size, row_size, board, empty_field, inverted);
    ...
    FINISH_PUZZLE:
        ...
        while (path[i] != -1) {
            ...
            play_turn(path[i], column_size, row_size, board, empty_field, 0);
            // Show board
            clear_screen();
            printf("Moved %s\n", get_direction_string(inverted ? get_opposite_direction(path[i]) : path[i]));
            show_board(column_size, row_size, board);
            turn_counter++;
            i++;
        }
