...
int get_priority(int moves_amount, int column_size, int row_size, int *board) {
    return manhattan(moves_amount, column_size, row_size, board);
}
...
int hamming(int moves_amount, int column_size, int row_size, int *board) {
    for (int i = 0, len = column_size * row_size; i < len; i++) {
        ...
        if (board[i] && board[i] != i+1) moves_amount++;
    }
    return moves_amount;
}
...
int manhattan(int moves_amount, int column_size, int row_size, int *board) {
    ...
    for (int i = 0, len = column_size * row_size; i < len; i++) {
        ...
        if (board[i] && board[i] != i+1) {
            ...
            moves_amount += distance(current_col, wanted_col) + distance(current_row, wanted_row);
        }
    }
    return moves_amount;
}